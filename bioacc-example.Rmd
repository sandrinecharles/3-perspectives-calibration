...
Perform calculations under the three perspectives as described within the main document *** to be developed ***

# Case study
Based on [@Ashauer2010]. Data set on *Gammarus pulex* exposed to Malathion. *** to be developed ***
...
A toxicokinetic (TK) model simply describing bioaccumulation of chemical substances within the whole body of living organisms is based on a set of two differential equations standing for the deterministic part [@Ratier2021]:
...
```{r, echo=FALSE, fig.cap="Simulation of a one-comparment toxicokinetic model with parameters values got from medians in Table 1 of [@Ashauer2010] (solid orange line). Black dots are observed data."}
# Plot simulations
ymax <- max(unique(df$conc)) # maximal observed concentrations
ggplot(simu.mean, aes(time, conc)) +
  geom_line(col = "orange", size = 1.5) +
  xlab("Time (hours)") +
  ylab("Measured internal concentrations") +
  geom_vline(xintercept = 1, linetype="dashed") +
...
```{r, echo=FALSE, fig.cap="Visual Predictive Check"}
